...
export default function UserPicker({ value, options, onSelect }) {
    ...
    const filteredPeople = query === "" ? options : options.filter((person) => person.name
        .toLowerCase()
        .replace(/\s+/g, "")
        ...
        <>
            <Combobox value={selected} onChange={onSelected} multiple>
                <div className="relative mt-1">
                    ...
                    focus-visible:ring-offset-indigo-300 sm:text-sm">
                        <Combobox.Input
                            className="border-gray-300 dark:border-gray-700 dark:bg-gray-900
                            dark:text-gray-300 focus:border-indigo-500
                            dark:focus:border-indigo-600 rounded-md shadow-sm mt-1 block w-full"
                    ...
                    <Transition
                        ...
                        afterLeave={() => setQuery("")}
                    >
                        <Combobox.Options className="absolute mt-1 max-h-60 w-full
                            ...
                            {filteredPeople.length === 0 && query !== "" ? (
                                ...
                                filteredPeople.map((person) => (
                                    <Combobox.Option
                                        key={person.id}
                                        className={({ active }) =>
                                            `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-indigo-600 text-white' : 'bg-gray-900 text-gray-100'
                                            }`}
                                        value={person}
                                    >
                                        {({ selected, active }) => (
            ...
            {selected && (
                <div className="flex flex-wrap gap-2 mt-3">
                    {selected.map((person) => (
                        <div  key={person.id} className="badge badge-primary p-3">{person.name}</div>
                    ))}
                </div>
            )}
        </>
